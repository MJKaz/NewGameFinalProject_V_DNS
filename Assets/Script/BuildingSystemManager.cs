...
public class BuildingSystemManager : MonoBehaviour
{
    ...
    private void ConfirmBuild()
    {
        ...
        // Check build cost first
        if (currentBuildableData.buildCost != null && currentBuildableData.buildCost.Length > 0)
        {
            ...
            if (!playerInventory.HasResources(currentBuildableData.buildCost))
            {
            ...
            {
                playerInventory.ConsumeResources(currentBuildableData.buildCost);
        ...
        if (Physics.Raycast(ray, out hit, maxBuildDistance, buildableLayer))
        {
            ...
            if (currentBuildableData != null && playerInventory != null)
            {
                foreach (var cost in currentBuildableData.buildCost)
                {
                    ...
                    // Drop physical pickups if prefab assigned
                    if (cost.pickupPrefab != null)
                    {
                        for (int i = 0; i < refundAmount; i++)
                        {
                            Vector3 dropPos = target.transform.position + Vector3.up * 0.5f + Random.insideUnitSphere * 0.25f;
                            GameObject drop = Instantiate(cost.pickupPrefab, dropPos, Quaternion.identity);
                            drop.GetComponent<Rigidbody>()?.AddForce(Vector3.up * 2f, ForceMode.Impulse);
                        }
                    }
                    else
                    {
